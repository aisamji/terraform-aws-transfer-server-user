variable "server_id" {
  description = "The server ID of the Transfer Server (e.g. `s-12345678`)"
}

variable "user1" {
  description = "A mapping describing the 1st user."
  default = {
    user_name = "aisamji"
    ssh_keys  = ["ssh-rsa  aisamji09@gmail.com"]
  }
}

variable "user2" {
  description = "A mapping describing the 2nd user."
  default = {
    user_name = "danny"
    ssh_keys  = ["ssh-rsa  aisamji09@gmail.com"]
  }
}

variable "bucket_name" {
  description = "The name of the S3 bucket that this user should have access to."
}

variable "group_home_dir" {
  description = "The *root* prefix that members of this group will have access to."
}
